...
window.socket.on('players-already-here', o=>{
  ...
  if(typeof window.config.movie != "undefined" && window.config.movie != "")
  window.setTimeout(()=>{
    document.querySelector('#movie').setAttribute('src',window.config.movie);
  },10000);
});
...
window.socket.on('set-spawn-data',data=>{
  if(window.gameHasBegun)window.setPlayerSpawnData(data);
  console.log('spawn data received: ');
  console.log(data);
});
